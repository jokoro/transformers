...
class AttentionLayer(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        # Y=dropout(softmax( d​ Q.K ⊤ +M​ ))V 
        dot_product = query @ key.transpose(1,2) / torch.sqrt(D)
        ...
        if attn_mask is not None:
            ...
            additive_mask = torch.clone(attn_mask)
            additive_mask[additive_mask==1] = 0
            additive_mask[additive_mask==0] = -float('inf')
            dot_product += additive_mask
        
        # apply softmax, dropout, and use value
        y = self.dropout(F.softmax(dot_product)) @ value
        return y  
...
class MultiHeadAttentionLayer(AttentionLayer):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        query = self.query_proj(query).view(N, S, H, -1).transpose(1,2)
        key = self.key_proj(key).view(N, T, H, -1).transpose(1,2)
        value = self.value_proj(value).view(N, T, H, -1).transpose(1,2)
        ...
        dot_product = query @ key.transpose(1,2) / torch.sqrt(D/H)
        ...
        output = self.head_proj(torch.cat(y.split(1,1), -1))
        return output
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        pos = torch.arange(S).tile(N,1)
        output = x + self.encoding(pos)
...
class CrossAttentionBlock(nn.Module):
    ...
    def forward(self, seq, cond):
        ...
        # Then add a residual connection to the original input, and finally apply normalization. #############################
        out = self.cross_attn(seq, seq, seq, cond) # query=decoder, key=encoder, value=encoder
        out = self.dropout(out)
        out = out + seq
        out = self.norm(out)
        return out
